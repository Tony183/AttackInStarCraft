...
class Trainer():
    ...
    def train(self, input, input_prev, real_val, idx=None):
        self.model.train()
        self.optimizer.zero_grad()
        output, adp, adp_prev, KLloss = self.model(input, input_prev, idx=idx)
        ...
        if self.iter % self.step == 0 and self.task_level <= self.seq_out_len:
            self.task_level += 1
        if self.cl:
            Entloss, correct = self.loss(predict[:, :, :, :self.task_level], real[:, :, :, :self.task_level])
        else:
            Entloss, correct = self.loss(predict, real)

        loss = Entloss + 0.03 * KLloss  # using the parameter in 2020 ICLR
        loss.backward()
        ...
        self.iter += 1
        return Entloss.item(), correct, KLloss.item()

    def eval(self, input, input_prev, real_val):
        self.model.eval()
        output, adp, adp_prev, KLloss = self.model(input, input_prev)
        # output = output.transpose(1, 3)
        # real = torch.squeeze(real_val, dim=1)
        real = real_val
        # predict = self.scaler.inverse_transform(output)
