...
parser.add_argument('--data', type=str, default='data/MARLActionPrediction/save/', help='data path')

parser.add_argument('--adj_data', type=str, default='data/sensor_graph/adj_mx_unix.pkl', help='adj data path')
...
parser.add_argument('--subgraph_size', type=int, default=11, help='k')
...
parser.add_argument('--in_dim', type=int, default=5, help='inputs dimension')
...
parser.add_argument('--save', type=str, default='./MARLActionPrediction/save/', help='save path')
parser.add_argument('--expid', type=int, default=0, help='experiment id: here 0 is normal model, 1 is adv model')
...
def main(runid):
    ...
    for i in range(1, args.epochs + 1):
        ...
        for iter, (x1, x2, y) in enumerate(dataloader['train_loader'].get_iterator()):
            ...
            for j in range(args.num_split):
                ...
                tx1 = trainx1[:, :, id.type(torch.long), :]
                tx2 = trainx2[:, :, id.type(torch.long), :]
                ty = trainy[:, :, id.type(torch.long), :]
                metrics = engine.train(tx1, tx2, ty, id)
                train_loss.append(metrics[0])
                train_mape.append(metrics[1])
                train_rmse.append(metrics[2])
            if iter % args.print_every == 0:
                log = 'Iter: {:03d}, Train Loss: {:.4f}, Train acc: {:.4f}, Train RMSE: {:.4f}'
        ...
        s1 = time.time()
        for iter, (x1, x2, y) in enumerate(dataloader['val_loader'].get_iterator()):
            ...
            metrics = engine.eval(testx1, testx2, testy)
        ...
        log = 'Epoch: {:03d}, Train Loss: {:.4f}, Train acc: {:.4f}, Train kl loss: {:.4f}, Valid Loss: {:.4f}, Valid acc: {:.4f}, Valid kl loss: {:.4f}, Training Time: {:.4f}/epoch'
    ...
    vmae, vmape, vrmse = metric(yhat, realy, adp, adp_prev)
